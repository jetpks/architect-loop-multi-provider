...
> **Two Claudes, one happy little loop!** (ﾉ◕ヮ◕)ﾉ*:･ﾟ✧
...
split into a repo-centered loop: specs and gates are written first, the builder
works in fresh contexts, and the architect reviews the evidence before *anything*
gets integrated. The best part? It runs on the Claude plan you already have — no
second CLI, no extra API keys! Yay! 🎉

## Install (30 seconds) 📦

```bash
git clone https://github.com/jetpks/architect-loop-multi-provider
cd architect-loop-multi-provider/opus-48-sonnet-46-kawaii && ./install.sh   # Windows: .\install.ps1
```
...
git-write deny rules plus that check stand in for it). Worktrees are discarded
and re-dispatched from the freeze commit.
