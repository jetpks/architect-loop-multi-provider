...
report the builder writes with its `Write` tool to `docs/lanes/<slice>-<lane>.md`.
...
multi-hour run (nobody likes waiting~):
...
own `docs/lanes/<slice>-<lane>.md`, so nothing collides — no stepping on toes! Claude Code has no `-C`,
...
( cd <repo-root>/.architect/wt/<slice>-<NN> && \
  claude -p --model claude-sonnet-4-6 \
    ...
    < <repo-root>/.architect/wt/<slice>-<NN>.block.md \
    > <repo-root>/.architect/wt/<slice>-<NN>.last-run.jsonl 2>&1 )
```
...
each other's files — personal space respected! Nothing reaches a branch until the architect's checks pass —
...
- Background each lane as its own harness task and let the **per-lane completion
  ...
  `.architect/last-run.jsonl` and the repo state afterwards. Do not write a
...
- Cross-model review gate (high-stakes slices): the architect is Opus 4.8 and the
  ...
  ```
- Add `.architect/` to the repo's `.gitignore` (don't forget!).
...
`tool_result` yet). Silent gaps between events are normal model thinking (just pondering~); a shell
...
Use when the human wants to babysit a run (sometimes you just wanna watch~).
