# DESIGN — The Architect Loop v2 (space-centric) ✨

> **A source-backed design for a Claude Code harness skill in which Opus 4.8 acts as architect/orchestrator and Sonnet 4.6 (run headless via `claude -p`, one tier down in the same harness) acts as builder, with the space-cadet space as the memory! (ﾉ◕ヮ◕)ﾉ*:･ﾟ✧**
...
> **Space-centric variant 🪐** — this variant runs inside a
> [space-cadet](https://github.com/jetpks/space-cadet) space, so **the space**
> (its own git repo) is the memory, and a single mission spans the repos cloned
> under `repos/`. Read every `docs/HANDOFF.md` / `docs/gates/` / `docs/lanes/` /
> `docs/prd/` below as the space-memory paths `architect/HANDOFF.md` /
> `architect/gates/` / `architect/lanes/` / `architect/prd/`, and every
> `.architect/` scratch path as `<space>/tmp/architect/` (gitignored by the
> space). Worktrees are added off a **target repo's** base commit but placed in
> the space's scratch; the **gate freeze** is a commit in the space repo. The
> invariants R1–R12 are unchanged — only their on-disk home moved from one repo
> to the space.
...
| **Memory** | the space: `architect/HANDOFF.md`, `architect/gates/`, the space's git history | permanent | everything; not in the space = didn't happen |
| **Repos** | the clones under `repos/`, each its own git repo | the work product | the code a mission touches; one mission spans several |
...
### R1. The space is the memory; not in `HANDOFF.md` = didn't happen 📝
The mission's memory lives in the space at `architect/` (`HANDOFF.md`, `gates/`,
`lanes/`, `prd/`), versioned by the space's own git repo and kept separate from
the code repos under `repos/`. Anthropic's long-running-agent harnesses use a
progress file + git history as the cross-session memory and find "compaction
alone is insufficient — structural artifacts are the load-bearing memory"
...
written to `architect/gates/<slice>.md` in the space before dispatch, committed
to the space repo, and the architect's post-run verification step includes
`git diff` on `architect/gates/` — **any change to a gate file after the freeze
is an automatic slice FAIL**, regardless of results. Here the blast-radius
separation is structural: builders run in worktrees of the code repos under
`repos/`, a different git repo from the space, and are never handed the gates
path. Criteria are quoted verbatim when judging, never restated from memory.
...
- **The PRD is space memory; raw findings are not.** `architect/prd/<slice>.md`
  is committed to the space with citations (R1); raw researcher output stays in
  the gitignored `<space>/tmp/architect/research/`. The builder's PHASE 0
  challenges the PRD like any other spec input.
...
| Context rot | Architect context holds judgment only; fresh builder process per slice; the space is the memory (R1, R7) |
