# architect-loop — kawaii · space-centric ✨

> **Two Claudes, one happy little loop — living in a space!** (ﾉ◕ヮ◕)ﾉ*:･ﾟ✧

**Opus 4.8 handles planning and review; Sonnet 4.6 handles implementation and
research — both in Claude Code!** 💖 Two adorable Claude Code skills wire that
split into a **space-centered loop**: you run inside a
[space-cadet](https://github.com/jetpks/space-cadet) space, and the space — its
own git repo — *is* the memory. Specs and gates are written first, the builder
works in fresh contexts in the repos under `repos/`, and the architect reviews
the evidence before *anything* gets integrated. One mission can span several
repos! The best part? It runs on the Claude plan you already have — no second
CLI, no extra API keys! Yay! 🎉

This is the **space-centric** variant: it assumes you're always standing in a
space-cadet space (`space new`, then work from inside it). For the plain
single-repo flow, use the `opus-48-sonnet-46-kawaii` variant instead.

## Install (30 seconds) 📦

```bash
git clone https://github.com/jetpks/architect-loop-multi-provider
cd architect-loop-multi-provider/opus-48-sonnet-46-kawaii-space && ./install.sh   # Windows: .\install.ps1
```

Here's the magic ✨ — both roles are the *same* `claude` binary! The architect is
your cozy interactive session, and the builder is headless `claude -p` one tier
down (Sonnet 4.6). Nothing else to install! 🚫 `./install.sh --project` installs
to the current repo only instead of globally. All you need is
[Claude Code](https://claude.com/claude-code) on any paid plan — that's it! 🎀 The
builder's `claude -p` hours draw on the Agent SDK credit pool on the same plan.
...
Super simple! `/architect` runs one work block: judge the last run, spec the next
slice, dispatch builders. 🚀 `/architect-research` is for when you're still
deciding *what* to build — its cited report feeds the build loop's PRD. Perfect
for: making confident decisions before you write a single line! 🎯
...
writes code! Why this rocks:

- **Spec + gates first.** 📝 The architect specs a one-PR slice, splits it into
  1–4 lanes (each naming its target repo) whose file sets are checked for
  overlap, and commits the acceptance gates to the space's `architect/gates/`
  *before* any builder starts. Gates are read-only and live outside the builders'
  worktrees; a post-freeze change fails the slice automatically. 🚫
...
- **The architect judges and integrates.** 🎯 It runs the gate commands itself
  ...
  review. 🔄
- **The space is the memory.** 💎 `architect/HANDOFF.md` (a short table of
  contents, pruned every session), `architect/gates/`, `architect/lanes/`,
  `architect/prd/`, all versioned by the space's own git repo. The clones under
  `repos/` and scratch under `tmp/` are gitignored by the space, so the loop's
  memory never pollutes the repos it touches. Not in the space = didn't happen!
...
Don't just take our word for it — each design choice is source-backed (full
...
| [skills/architect/HANDOFF.template.md](skills/architect/HANDOFF.template.md) | The space-memory file (incl. repos-in-scope) |
...
**Do I need a second CLI or API keys?** Nope! 🎉 Both the architect and the builder
...
**What if a builder wrecks things?** Don't worry! 🛡️ Nothing reaches a branch
until the architect's tamper, boundary, and gate checks pass — including a
`git log` check that the lane made no commits (Claude Code has no sandbox, so
git-write deny rules plus that check stand in for it). Worktrees live in the
space's scratch (`tmp/architect/`) and are discarded and re-dispatched from the
target repo's base commit.
