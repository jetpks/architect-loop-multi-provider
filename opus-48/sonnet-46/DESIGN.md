...
as architect/orchestrator and Sonnet 4.6 (run headless via `claude -p`, one tier
down in the same harness) acts as builder, with the repo as the only memory.**
...
the Codex CLI; the methodology is model-agnostic and has been retargeted to an
all-Claude-Code Opus 4.8 / Sonnet 4.6 pairing — the invariant rules R1–R12 are
unchanged.)
...
cross-vendor bias reduction the original Fable 5 / GPT-5.5 pairing leaned on. Be
honest about what survives the retarget: the load-bearing separations in the
...
## 4. The builder interface (verified against the `claude` CLI / Claude Code headless mode, June 2026)
