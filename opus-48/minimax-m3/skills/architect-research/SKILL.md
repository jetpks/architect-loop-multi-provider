...
Write each lane block to a `.prompt.md` file and pass it with `@file` — never
as a shell argument; `@file` injects the contents verbatim past quote-mangling
shells.

(Web comes from the `pi-web-access` extension: `web_search` returns a
synthesized answer with source citations, `fetch_content` pulls a page/PDF/repo.
Zero-config via Exa — no key needed. Launch ONE canary lane and confirm it
actually fetches live URLs before fanning out. The lane blocks also run verbatim
as read-only Claude subagents with web search if you'd rather keep research in
the architect's own harness.)
