...
- SKILL.md frontmatter description > 1024 chars -> harnesses reject the skill
  (observed live: "invalid description: exceeds maximum length").
    ...
    if not desc:
        ...
        if len(flat) > MAX_DESC:
            errors.append(
                ...
                "(harness refuses to load the skill)"
            )
...
def check_siblings(skill_dir: Path) -> None:
    ...
    for ref in re.findall(r"`([\w][\w.-]*\.md)`", skill_md):
        ...
        if re.match(r"(docs|artifacts|lane|gate|prd|research)", ref):
            continue
